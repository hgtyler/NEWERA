...
function onKeyUpEscape(event) {
  if (event.key === "Escape") {
    // Logic to handle the escape key, such as closing a modal or dropdown
    const activeElement = document.activeElement;
    if (activeElement) {
      activeElement.blur(); // Example: Unfocus the current active element
    }
  }
}
...
class FacetFiltersForm extends HTMLElement {
  ...
  static renderProductGridContainer(html) {
    ...
    // Make sure the loading state is removed
    document
      .getElementById("ProductGridContainer")
      .querySelector(".collection")
  ...
  static renderAdditionalElements(html) {
    ...
    const menuDrawer = document
      .getElementById("FacetFiltersFormMobile")
      .closest("menu-drawer");

    if (menuDrawer) {
      // Check if bindEvents exists and is a function before calling it
      if (typeof menuDrawer.bindEvents === "function") {
        menuDrawer.bindEvents();
      } else {
        console.warn(
          "Menu drawer found, but bindEvents function not present or necessary.",
        );
      }
    } else {
      console.warn("Menu drawer element not found.");
    }
  }
